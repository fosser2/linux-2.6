...
int smba_bt_wifi_gpio_init(void)
{
	...
	gpio_request(SMBA9701_WLAN_POWER, "bt_wifi_power");
...
int smba_bt_wifi_gpio_set(bool on)
{
	...
	if (on) {
		if (count == 0) {
			gpio_set_value(SMBA9701_WLAN_POWER, 1);
        		mdelay(100);
		...
		if (count == 0) {
			...
			return -1;
			        ...
			        gpio_set_value(SMBA9701_WLAN_POWER, 0);
        			mdelay(100);
...
void smba_gps_mag_poweron(void)
{
	if (atomic_inc_return(&smba_gps_mag_powered) == 1) {
		...
		gpio_set_value(SMBA9701_GPSMAG_DISABLE, 1); /* Enable power */
		msleep(2);
	}
}
...
static void __init tegra_smba_init(void)
{
	...
	/* Finally, init the external memory controller and memory frequency scaling
   	   NB: This is not working on SMBA9701. And seems there is no point in fixing it,
